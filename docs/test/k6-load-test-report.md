...
| Threshold | 기준 | 실측값 | 판정 |
|-----------|------|--------|------|
| search_duration p(95) | < 300ms | 4.8ms | PASS |
| search_duration p(99) | < 500ms | 109.77ms | PASS |
| detail_duration p(95) | < 300ms | 4.62ms | PASS |
| detail_duration p(99) | < 500ms | 119.83ms | PASS |
| rate_duration p(95) | < 500ms | 8.2ms | PASS |
| rate_duration p(99) | < 1000ms | 172.17ms | PASS |
| reservation_duration p(95) | < 1000ms | 22.17ms | PASS |
| reservation_duration p(99) | < 2000ms | 188.21ms | PASS |
...
|---------|-----|-----|-------|-------|-------|-------|-----|
| 검색 (search) | 3.71 | 0.27 | 0.79 | 2.50 | 4.80 | 109.77 | 487.54 |
| 상세 (detail) | 3.93 | 0.33 | 0.87 | 2.73 | 4.62 | 119.83 | 464.96 |
| 요금 (rate) | 6.62 | 0.97 | 2.25 | 5.62 | 8.20 | 172.17 | 554.75 |
| 예약 (reservation) | 9.26 | 1.14 | 2.81 | 10.14 | 22.17 | 188.21 | 660.04 |
...
|------|-----|
| 총 요청 수 | 34,675건 |
| 처리량 (RPS) | 431.57 req/s |
| 총 반복 수 | 27,265회 |
| 테스트 시간 | 80.3s |
| 수신 데이터 | 48 MB (601 KB/s) |
...
재고 20개 x 10개 객실 = 총 200건의 예약이 정확히 성공하고, 이후 요청은 모두 재고 부족(INVENTORY_INSUFFICIENT)으로 처리되었다.
...
| rate | 3분 | roomType별 날짜 범위 요금 | roomTypeId prefix 기반 evict |
| inventory | 미캐싱 | 재고 | 정합성 우선, 매번 DB 조회 |

### 캐시 효과 분석

- 검색 p(50) 0.79ms: 캐시 히트 시 sub-millisecond 응답
- 상세 조회 p(50) 0.87ms: property + roomTypes 캐시 활용
- 요금 조회 p(50) 2.25ms: rate 캐시 히트 + inventory DB 조회 포함
- 예약 p(50) 2.81ms: 캐시 미사용(쓰기 트랜잭션), 재고 비관적 잠금 포함
