...
- Google Java Style Guide를 기반으로 하며, 들여쓰기는 4 스페이스를 사용한다. (intellij-java-google-style.xml 참고)
...
- 문자열, 숫자 등 매직 값은 하드코딩하지 않고 `private static final` 상수로 추출한다
- 역할(role), 컨텍스트(context), 단위 변환 값 등이 대상이다

```java
// 올바름
private static final String ROLE_PARTNER = "PARTNER";
private static final long SECONDS_DIVISOR = 1000;
jwtProvider.generateToken(id, ROLE_PARTNER, CONTEXT_PARTNER);

// 잘못됨
jwtProvider.generateToken(id, "PARTNER", "PARTNER");
jwtProvider.getExpiration() / 1000;
```

### 메서드 파라미터 개행 원칙

- Controller, Docs 인터페이스, record의 메서드 파라미터는 파라미터가 1개라도 별도 줄로 분리한다
- 호출부가 아닌 선언부(메서드 시그니처)에 적용한다
...
// 잘못됨
public ApiBaseResponse<UserResponse> getMe(final UserId userId) {
    ...
}
```
