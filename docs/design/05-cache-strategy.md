...
flowchart TD
    A[숙소 요금 조회 API 요청] --> B
    B["1. property 캐시 조회\n@Cacheable, 히트 시 즉시 반환"] --> C
    C["2. roomTypes 캐시 조회\n@Cacheable, 히트 시 즉시 반환"] --> D
    D["3. 요금 Bulk IN 쿼리 1회\nper-date warm-up 캐시 적재"] --> E
    E["4. 재고 Bulk IN 쿼리\n항상 DB 실시간 조회"] --> F
    F["5. 날짜별 요금+재고 조립\n응답 반환"]
```
...
|---------|-------------|-------------|--------|
| 검색 | 109.77ms (0.110s) | 9.97ms (0.010s) | -91% |
| 상세 | 119.83ms (0.120s) | 10.42ms (0.010s) | -91% |
| 요금 | 172.17ms (0.172s) | 26.03ms (0.026s) | -85% |
| 예약 | 188.21ms (0.188s) | 26.81ms (0.027s) | -86% |
...
flowchart TD
    A["파트너가 요금 수정\nExtranet API"] --> B
    B["DB UPDATE\n@Transactional"] --> C
    C["RateUpdatedEvent 발행"] --> D
    D["CacheEvictListener\nAFTER_COMMIT 해당 날짜 evict"] --> E
    E["다음 조회 시\nDB 최신 요금 → warm-up 재적재"]
```
